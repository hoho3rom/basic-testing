...
describe('BankAccount', () => {
  ...
  test('should throw error when transferring more than balance', () => {
    ...
    expect(() => account.transfer(initialBalance + 1, recipient)).toThrow(InsufficientFundsError);
  });

  test('should throw error when transferring to the same account', () => {
    expect(() => account.transfer(initialBalance + 1, account)).toThrow(TransferFailedError);
  });

  test('should deposit money', () => {
    const deposit = 50;
    expect(account.deposit(deposit).getBalance()).toBe(initialBalance + deposit);
  });

  test('should withdraw money', () => {
    const withdraw = 50;
    expect(account.withdraw(withdraw).getBalance()).toBe(initialBalance - withdraw);
  });
  ...
  test('should set new balance if fetchBalance returned number', async () => {
    ...
    jest.spyOn(account, 'fetchBalance').mockReturnValue(Promise.resolve(balance));
